...
| التحليل المحلي | FFprobe + مرشحات FFmpeg | لا يتطلب API ولا رفع فيديو؛ يكشف الصمت، حدود المشاهد، متوسط/أقصى مستوى صوت، والبيانات الوصفية. حدود المشاهد ليست أوصافًا دلالية للصورة، ولا يوجد نموذج رؤية محلي مضمّن. |
| فهم بصري اختياري | FFmpeg لاستخراج JPEG منخفض الدقة + Gemini Vision | بعد ضغط زر العين وموافقة منفصلة فقط؛ نحو عينة لكل ثانية وإطارات إضافية للقطات دون الثانية. يرسل صورًا ثابتة لا ملف الفيديو/الصوت، ويحذف cache الصور بعد الطلب؛ تحفظ الأوصاف والملخصات والتوقيتات في `AnalysisResult.visualIndex`. جودة/تكامل Gemini الحقيقي وWindows غير مثبتين هنا. |
...
- **Google Gemini API:** اختياري ويتطلب إدخال المستخدم للمفتاح من Settings. يُحفظ المفتاح مشفرًا عبر Electron `safeStorage` في ملف منفصل داخل `app.getPath('userData')` (على Windows يستخدم DPAPI)، ولا يمر إلى الـrenderer بعد الحفظ ولا يُكتب في Settings JSON أو السجلات. تستخدم الشبكة رأس `x-goog-api-key` لا query string. ترسل الجولة العادية طلب المستخدم فقط؛ يطلب النموذج أدوات سياق محلية حسب حاجته، ثم تُرسل نتائج الأدوات المحددة (وقد تتضمن نص التفريغ/العناوين أو أوصاف الفهرس البصري عند السؤال عنها) إلى Gemini. المسارات المحلية وملف الفيديو/الصوت الخام لا تُرسل في المحادثة أو تلقائيًا. فهرسة الرؤية مسار منفصل: يعرض renderer تقدير المدة وعدد الإطارات، ويطلب Main IPC قيمة موافقة صريحة قبل استخراج/إرسال أي صورة؛ تُرسل إطارات JPEG منخفضة الدقة فقط، ثم تُحذف النسخ المؤقتة. أخطاء المزود تُختزل إلى رموز عامة ولا تشمل المفتاح.
...
7. فهرسة الصور لا تبدأ من Function Calling ولا من أداة Agent؛ زر بصري يفتح نافذة موافقة فيها تقدير العينات والمدة. يتحقق IPC من `consent === true` قبل قراءة المفتاح أو استخراج الإطارات، ولا يُرسل ملف المصدر. هذه إشارة موافقة للتدفق الطبيعي داخل التطبيق وليست حماية من مستخدم يعبث عمدًا بـDevTools/البرنامج.
...
`MediaAsset` يخزن مسار الأصل، المدة، الأبعاد، FPS، codec وحالة الصوت. `AnalysisResult` يحوي `scenes`, `silences`, `transcript` (كلمات إن وفّرها Whisper)، `audioMetrics`, `quality`, warnings وتوقيت التحليل، ويحتوي اختياريًا `visualIndex` بملخص وتوقيتات/أوصاف لكل عينة ولقطة؛ لا تحفظ الصور المرسلة نفسها. مسار الأصل يبقى مرجعًا مطلقًا لتجنب نسخه، وإن تغير يدعم التطبيق Relink؛ أما الصور المؤقتة للفهرسة فتُنظف بعد التحليل.
...
| اكتشاف ثنائيات FFmpeg/FFprobe وتشغيل `spawn` بدون shell | **Implemented · Requires FFmpeg** | `npm run test:media` توقف برسالة واضحة لعدم توفر FFmpeg قابل للتشغيل قبل إنشاء الوسائط؛ لم يحدث تصدير حقيقي. |
...
| Agent Gemini وأدواته | **Implemented · Unit-tested with mocks · Live API not tested** | الاختبارات الوهمية تثبت حلقة الأدوات فقط. لم يُرسل طلب حي إلى Gemini في هذا التشغيل. |
...
| Shorts candidates | **Experimental** | ranking من transcript وحدود الصمت/المشاهد وخصائص زمنية؛ لا يثبت احتمالية الانتشار، ولا يستهلك الفهرس البصري. |
| فهرسة بصرية اختيارية والاستعلام عن التوقيت | **Implemented in code · Mock/unit-tested · No live Gemini or Windows verification** | اختبارات التخطيط وربط tool وIPC consent لا تثبت جودة captions من النموذج أو FFmpeg الحقيقي على Windows. الفهرسة ترسل إطارات فقط بعد تأكيد صريح. |
| Smart Crop (content-aware reframing لكل مقطع عند التصدير) | **Implemented · Unit tested** | `src/main/cropPlanner.ts` فوق `smartcrop.js` (MIT) + `extractRawFrameAt`؛ تراجع تلقائي للقص المركزي. `npm run test:media` يغطي مسار التصدير على Windows. |
| Object Tracking، Text Layers، Transitions/Effects | **Not implemented** | لا يوجد تنفيذ أو اختبار لهذه الميزات في هذا الفرع. |
| استيراد ترجمة SRT/VTT | **Implemented · Unit tested** | `src/main/subtitleImport.ts` فوق `@plussub/srt-vtt-parser` (MIT) + `importSubtitles` (تعديل واحد قابل للتراجع) + زر استيراد في محرر الترجمة. |

التحقق في 2026-09-23: `npm run typecheck` و`npm run build` و`git diff --check` نجحت على Linux. `npm test` سجّل **54 passed, 3 skipped**؛ المتخطاة تكاملا FFmpeg وطلب Gemini الحي. أضيفت اختبارات mock لتخطيط عينات الرؤية، حفظ الأوصاف المؤقتة وتنظيفها، تحويل توقيت Agent، وحارس موافقة IPC. `npm run test:media` توقف قبل التشغيل الحقيقي لغياب FFmpeg قابل للتشغيل. لم يحدث اتصال حي بـGemini، ولم يُبنَ أو يُثبت التطبيق على Windows، ولم يُختبر عرض واجهة Electron أو جودة captions من النموذج. هذه ليست ادعاءات نجاح للتصدير أو Windows أو تكامل الرؤية الحقيقي.
...
خيار رؤية محلي بالكامل دون خدمة سحابية، كاش waveform/إطارات أدق، قوالب اجتماعية متقدمة وtracking/smart crop مع إظهار المصدر والثقة وعدم اختلاق أوصاف.
